...
final public class OpenAlexFormatParser extends MAGFormatParser {

    public OpenAlexFormatParser(String sourceDir, String targetDir,
       String csvPrefix) {
    ...
    public void readFile() {
      ...
      try  {
        ...
        papersCSV.println("paper_id~rank~doi~doc_type~genre~paper_title~original_title~book_title~year~date~publisher~journal_id~conference_series_id~conference_instance_id~volume~issue~first_page~last_page~reference_count~citation_count~estimated_citation~original_venue~oa_status~doi_lower~created_date");
    ...
    private void parseFieldOfStudyChildrenFile(String path, PrintWriter fieldOfStudyChildrenCSV) {
      try {
        BufferedReader br = Files.newBufferedReader(Paths.get(path), StandardCharsets.UTF_8);
        // Skip header
        br.readLine();
	    ...
	    // I am checking for the foreign key constraint here	
            if (fos1 != null && !fos1.equals("") && fos2 != null && !fos2.equals("")) {
              String fieldOfStudyChildrenContent = fos1 + "~" +
                                	   	   fos2;
              fieldOfStudyChildrenCSV.println(fieldOfStudyChildrenContent);
            }
          }
        }
        fieldOfStudyChildrenCSV.flush();
        br.close();
      } catch (Exception e) {
        e.printStackTrace();
      }
